const polkadotTypes = (function (exports, util, utilCrypto) {
  ...
  const definitions$c = {
    rpc: {},
    types: util.objectSpread({}, location, xcm, v0$1, v1$1, v2, mapXcmTypes(XCM_LATEST), {
      DoubleEncodedCall: {
        encoded: 'Vec<u8>'
      },
      XcmOriginKind: {
  ...
  */
  ...
  class Enum {
    ...
    static with(Types) {
      ...
      for (let i = 0; i < keys.length; i++) {
        const name = util.stringUpperFirst(util.stringCamelCase(keys[i].replace(' ', '_')));
        asKeys[i] = `as${name}`;
        isKeys[i] = `is${name}`;
      }
  ...
  class CodecSet extends Set {
    ...
    static with(values, bitLength) {
      ...
      for (let i = 0; i < keys.length; i++) {
        isKeys[i] = `is${util.stringUpperFirst(util.stringCamelCase(keys[i]))}`;
      }
  ...
  const allExtensions = util.objectSpread({}, substrate$1, polkadot, shell, statemint); // the v4 signed extensions prior to the point of exposing these to the metadata.
  ...
  function expandExtensionTypes(extensions, type, userExtensions = {}) {
    ...
    .map(key => userExtensions[key] || allExtensions[key]).filter(info => !!info).reduce((result, info) => util.objectSpread(result, info[type]), {});
  }
  ...
  class PortableRegistry extends Struct {
    constructor(registry, value) {
      ...
      _classPrivateFieldBase(this, _types)[_types] = extractTypeMap(this.types); // console.timeEnd('PortableRegistry')
    }
